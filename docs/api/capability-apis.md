...
| `model_tool.calendar.create_recurring_event` | implemented | `src/tool-contract` | 创建 daily / weekly 重复日程 | 可写飞书日历；使用飞书原生 recurrence；不展开无限微信提醒 |
...
| `model_tool.calendar.update_and_create_events` | implemented | `src/tool-contract` | 同一句里先修改已有日程，再创建明确时间的新日程 | 可写日历；先执行修改，再执行创建；可刷新或登记提醒 |
...
| `model_tool.assistant.remember_todo` | implemented | `src/tool-contract` | 记录待推进事项，可携带日期和粗时段 | 可写 Seed Lite，可按粗时段自动安排安全时间 |
...
| `model_tool.calendar.daily_briefing` | implemented | `src/tool-contract` | 生成早报或晚报；用户明确问某一天安排时可带 `date` | 读日历；可更新 Seed Lite 拉回次数、搁置状态和分组 Watchlist 短期状态 |
...
- `assistant.remember_todo.preferredWindow` 可在自动安排待推进事项时保留“今天下午”这类粗时段，后续写日历时不会落到上午。
...
- `calendar.create_event`、`calendar.create_recurring_event`、`calendar.create_events` 和 `calendar.create_and_propose_schedule.events[]` 必须带 `startTimeEvidence`，且该片段必须来自用户原文；证据比较会容忍空白和全角/半角差异，但不会把系统推荐时间当作用户原文时间；如果时间是系统推荐出来的，只能先走排程推荐，不能直接写日历。新建飞书日程未显式传提醒时，会默认写入 40 分钟原生提醒；用户明确不用提醒时传 `reminderMinutes: 0`，飞书提醒和微信提醒都会关闭。
- `startTimeEvidence` 还必须能对应结构化 `startTime`。例如用户只说“明天和产品聊一下”时，模型不能用“明天”作为 `09:00` 的开始时间证据；应转为追问或排程推荐。
- `calendar.create_recurring_event` 只接受结构化 `recurrence`，不接受裸 `RRULE` / `rrule`；当前支持 `daily`、`weekly` 和可选 `count`，weekly 必须带 `byWeekday`。重复提醒可带 `reminderAtStart: true`，飞书 payload 会写入 0 分钟原生提醒；如果引用 Seed Lite `sourceIds`，创建成功后才移出待推进。首期只支持创建重复日程，不支持修改或删除重复系列。重复日程的首个实例会走同一套冲突检查和周几日期护栏，但不会展开成无限微信提醒队列。
...
- `event_query.title` 在已经带日期和时间约束时支持短别名匹配，例如用户说“天启会议”可匹配同一时间下的“跃为-天启 极刻光核项目沟通”；没有日期或时间约束时仍只做严格包含，避免误删。
...
- `calendar.update_and_create_events` 用于同一句里既改旧日程、又新增明确时间日程的情况；执行层固定先修改、再创建，避免把要修改的旧会新增成重复日程，也避免“明天下午两点”这类明确时间被误走排程候选。
...
- `calendar.daily_briefing.date` 可表达“明天安排 / 某天工作台”这类目标日期；未传 `date` 时仍沿用早报查今天、晚报查明天的旧行为。
