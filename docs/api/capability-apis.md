...
工程师朋友的建议是对的。Navi 现在已经不是单个日历功能，而是一组私人助手能力：日历、待推进、排程推荐、做梦记忆、早晚报、提醒和运行体检。下一步最重要的不是继续堆 prompt，而是把这些能力整理成稳定 API 合同，让后续 AI 先查能力目录，再决定调用哪个入口。

当前做法是：`src/capability-api/index.ts` 作为机器可读能力目录和聚合 facade；本文作为人和 AI 都能读懂的说明。真实用户消息可以走 `assistant.core`，它内部仍复用 `assistant.handle_message`；模型可见工具仍来自 `src/tool-contract`，不会新增正则路由或第二套日历写入路径。P30.3 后，能力目录只登记已实现能力，未来设想不进入机器可读 API surface。

## CEO 视角

这件事的价值不在“代码更好看”，而在让 Navi 从功能链路变成可长期维护的个人时间系统。

- 用户体验上，未来 AI 不需要猜系统有哪些能力，可以直接查 `capability-api`。
- 产品信任上，每个能力都写清楚能不能写日历、会不会发微信、失败时怎么回。
- 演进节奏上，先固化能力边界，再考虑更复杂的模型编排；真实回归没过的两段路由已经从活跃源码移除，避免冒充稳定能力。

本阶段不做 Trigger、后台读屏、多用户后台，也不把实验路由接到生产默认入口。

## ENG 视角

现有代码已经有几层 API，但以前分散在不同目录：

- `src/agent-api/index.ts`：微信和 shadow route 的主入口。
- `src/tool-contract`：模型可见工具合同。
- `src/calendar-api`：确定性日历执行内核。
- `src/seed-lite`：待推进收件箱。
- `src/memory-dream`：每日本地记忆整理。
- `src/wechat-reminder`：微信提醒队列。
- `src/live/*`：早晚报、提醒、体检等运行任务。

P30 补的是统一索引和文档，P30.1 补的是少数可调用 facade，而不是改语义判断方式。P30.2 已把两段式路由实验从活跃源码、公共导出和能力目录移除；P30.3 进一步收紧状态类型，能力目录只允许 `implemented`。真实路径继续保持单次模型工具决策。
