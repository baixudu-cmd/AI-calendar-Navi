...
export const TOOL_SCHEMAS: Record<CalendarToolName, ToolSchema> = {
  "calendar.create_event": {
    ...
    parameters: {
      ...
      properties: {
        ...
        reminderMinutes: { anyOf: [{ type: "number" }, { type: "array", items: { type: "number" } }] },
  ...
  "assistant.manage_todos": {
    ...
    parameters: {
      ...
      required: ["operation"],
      properties: {
        operation: { type: "string", enum: ["list", "complete", "delete", "update"] },
        limit: { type: "number" },
        target: {
          type: "object",
          required: [],
  ...
  "assistant.status_overview": {
    ...
    description: "只读总结当前还挂着的待处理上下文，例如待补时间、待确认推荐、待确认删除和待推进事项。",
    parameters: {
      type: "object",
      required: [],
      properties: {},
