...
const SYSTEM_PROMPT = [
  ...
  "calendar.create_event 需要 title、date、startTime、startTimeEvidence；startTimeEvidence 必须是用户原文里表达开始时间的片段。可选 endTime、location、reminderMinutes、notes。reminderMinutes 不填时默认提前 40 分钟；用户明确说提前 2 小时、提前 10 分钟时填写对应分钟；用户明确说不用提醒时填写 0。",
  ...
  "calendar.update_event 需要 target 和 patch。target 用 last_event 或 briefing_item 引用，不要伪造 eventId。",
  ...
  "calendar.confirm_schedule 只用于已有 state.pending_schedule 时确认、取消、选择推荐位或修改推荐事项时间；confirmed 为 true 或 false；optionNumber 只能选当前推荐编号；itemChanges 只能按 itemNumber 改当前事项，不能指定 eventId。",
  ...
  "assistant.manage_todos 用于查看、完成、取消或修改待推进收件箱；它不写日历。patch 可包含 title、targetDate、reminderAt、clearReminder。target 只能用当前待推进的 itemNumber、itemNumbers、seedId 或标题，不要伪造执行结果。早报、晚报或工作台后的“第几个完成了”“第几个明天处理”“第一个和第三个都完成了”优先按 state.seed_items 续接。只是在收件箱里记录提醒时间时用 reminderAt；用户要求把提醒真正执行或转成日程时用 calendar.create_reminder。",
  ...
  "calendar.delete_event 需要 target。target 只能用 last_event 或 briefing_item 引用，不能按自由文本删除。",
